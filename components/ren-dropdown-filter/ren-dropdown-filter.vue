        ...
        <view class="inner-wrapper">
            ...
            <view class="navs">
                <view class="c-flex-align" :class="{ 'c-flex-center': index >= 0, actNav: index === actNav }" v-for="(item, index) in navData" :key="index" @click="navClick(index)">
                    ...
                    <image src="../../static/icon/QsHxlr1gbSImvWt.png" mode="" class="icon-triangle" v-if="index === actNav"></image>
                    <image src="../../static/icon/xjVSvzWcH9NO7al.png" mode="" class="icon-triangle" v-else></image>
                </view>
                ...
                <!-- <view class="date-wrapper">
                    <picker mode="date" @change="handleDate">
                        <view class="date c-flex-align" :style="{ height: height + 'rpx' }" @click="dateClick">
                            <view>{{ selDate }}</view>
                            <image src="../../static/icon/xjVSvzWcH9NO7al.png" mode="" class="icon-triangle"></image>
                        </view>
                    </picker>
                </view> -->
				<view v-if="show_address" class="c-flex-align c-flex-center">
					<view class="c-flex-align" :style="{ height: height + 'rpx' }" @click="addressShow = true">
						<view>地址</view>
						<image src="../../static/icon/xjVSvzWcH9NO7al.png" mode="" class="icon-triangle"></image>
					</view>
				</view>
...
export default {
    ...
    methods: {
		...
		setinitial(defaultIndex){
			let init_defaultIndex = [];
			for(let i=0;i<defaultIndex.length;i++){
				let di;
				if( i== 0 ){
					di = defaultIndex[i]+1
				} else {
		...
		addressConfirm(e) {
			...
			let d = e.province.label + '-' + e.city.label + '-' + e.area.label;
