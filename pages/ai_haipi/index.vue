<template>
	<view class="indexContent">
		...
		<view v-if="current == 0" class="product-list">
			<view v-for="(item, index) in tabs[0]['goods']" :key="index" class="goodsproduct product" @click.stop="toGoodsDetail(item.id)">
				...
				<view style="padding: 0 4% 10rpx; font-size: 24rpx; display: flex;">
					<view  @click.stop="toGoodsPage(item.companyid)">{{ item.company_name }} 进店></view>
				</view>
			</view>
		</view>
		<view class="product-list" v-if="current != 0 && tabs[current]['goods'] && tabs[current]['goods'].length > 0">
			<view v-for="(item, index) in tabs[current]['goods']" :key="index" class="shopproduct product" @click.stop="toGoodsPage(item.id)">
				...
				<view class="shop-box">
					...
					<view style="overflow: hidden; padding-left: 15rpx;" v-if="item['goods_list'] && item['goods_list'].length > 0">
						<view style="width: 229rpx; float: left; padding-right: 15rpx;" v-for="(it,ins) in item['goods_list']" :key="ins" @click.stop="toGoodsDetail(it.goods_id)">
	...
	onLoad() {
		...
		uni.getStorage({
		    ...
		    success: function (res) {
				...
				_this.init_list({'page':1,'shop_type':1});
		    },
			fail(res) {
				// 如果没有经纬
				uni.authorize({
					...
					fail() {
						_this.$refs.uToast.show({
							title: "请先允许小程序获取您的位置信息，才能推送相关内容",
							callback(){
								uni.switchTab({
	...
	onReachBottom(){
		...
		if(this.current == 0){
			...
			if(maxpage > page) {
				...
				this.goods_recommend_list({'page':page+1,'shop_type':1},function(msg){
			...
			if(maxpage > page) {
				currTab.page = page+1;
				this.getCompanyList({'page':page+1,'shop_type':1},function(msg){
					_this.concatGoods(msg);
				})
			} else {
				_this.$refs.uToast.show({
	...
	methods: {
		sectionChange(index) {
			...
			if(index == 0) {
				...
				let data = {'page':1,'cateid':cateid,'shop_type':1};
