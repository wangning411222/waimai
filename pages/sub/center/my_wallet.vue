      ...
      <view class="scrollView">
        <view class="head-read">
          <view class="flex">
            ...
            <!-- <view class="arrow arrow-one" ><span>名词解释</span></view> -->
          </view>
        </view>
        <view class="white-box">
          <view class="tx">
            ...
            <view class="tx-grid" @click="navTo('/pages/sub/center/withdraw')"><span class="tx-grid-comm-sign">立即提现</span>
            </view>
          </view>
        ...
        <view class="top">
          <tui-tabs :tabs="tabs" :height="88" :currentTab="currentTab" :sliderWidth="150" :sliderHeight="60" bottom="50%"
            itemWidth="50%" color="#888" selectedColor="#fff" sliderBgColor="#ff8a4a" @change="changeTab"></tui-tabs>
        </view>
        <view class="list-view" v-if="list.length > 0">
          <view class="list-item" v-for="(item, index) in list" :key="index" hover-class="hover" :hover-stay-time="150"
            bindtap="detail">
            <view class="content-box">
              <view class="des-box">
                <view class="tit">{{
                  currentTab == 1
                  ? "流水号：" + (90000000 + Number(item.withdrawid))
                  : "订单号：" + item.orderid
                }}</view>
                <view v-if="currentTab == 1" style="margin-top: 12rpx">
                  账号：{{ item.ali_account }}
                </view>
                <view v-if="currentTab == 1" class="source" :style="{
                  color:
                    item.status == 1
                      ? '#4caf50'
                      : item.status == 2
...
export default {
  ...
  methods: {
    ...
    getFundList() {
      let _this = this;
      if (this.currentTab == 1) {
        this.$store.dispatch("user/withdrawList", {}).then((res) => {
          if (res.code == 0) {
        ...
        this.$store.dispatch("user/moneyLog", {}).then((res) => {
          if (res.code == 0) {
            _this.list = res.message;
          } else {
            _this.$refs.uToast.show({
