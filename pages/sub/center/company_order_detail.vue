	...
	<view class="order-detail" v-if="JSON.stringify(order) != '{}'">
		<view class="card">
			...
			<view v-if="order.product_sku[0]['deliver_way'] == 1 && order.status == 2" style="padding-top: 15rpx;">
				运单编号：{{ order.waybill }} <text @click="fuzhi_yundan(order.waybill)"
					style="margin-left: 10rpx; padding: 0 5rpx; border: 1rpx solid #CCCCCC;font-size: 24rpx;">复制</text>
			</view>
		</view>
		...
		<view class="order">
			...
			<view class="total">
				...
				<text class="total-price">
					￥{{ order.total_rmbs_fmt }}
				</text>
			</view>
			<view class="bottom">
				<view style="flex: 1;"></view>
				<view v-if="order.status == 1 || order.status == 9" class="evaluate btn margin-left-10"
					@tap="refund(order.orderid)">退款</view>
				<view v-if="order.status == 1 || order.status == 2 || order.status == 3 || order.status == 4"
					class="evaluate btn margin-left-10" @tap="print(order.order_token)">打印小票</view>
				<view v-if="order.status == 9" class="evaluate btn margin-left-10" @tap="companyOrderUpdate(order.orderid)">
					手动接单</view>
...
export default {
	...
	methods: {
		getOrderList(orderid) {
			let _this = this;
			_this.$store.dispatch('user/orderRead', { 'orderid': orderid }).then((res) => {
				if (res.code == 0) {
		...
		refund(orderid) {
			...
			uni.showModal({
				...
				success: function (res) {
					if (res.confirm) {
						_this.$store.dispatch('user/companyRefund', { 'orderid': orderid }).then((res) => {
							if (res.code == 0) {
...
.estimated-revenue-top,.estimated-revenue-bottom{
	display:flex;
	flex-direction: row;
