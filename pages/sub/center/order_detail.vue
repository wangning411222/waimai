	...
	<view class="order-detail" v-if="JSON.stringify(order) != '{}'">
		...
		<view class="order">
			...
			<view class="bottom">
				...
				<view v-if="order.status == 0" class="evaluate btn margin-left-20"
					@click.stop="cancelOrder(order.orderid)">
					取消订单</view>
				<view v-if="order.status == 4" class="evaluate btn">确认收货</view>

				<view v-if="order.status == 9" class="evaluate btn margin-left-10" @tap="refund(order.orderid)">退款
				</view>
			</view>
		</view>
