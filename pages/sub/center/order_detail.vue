	...
	<view class="order-detail" v-if="JSON.stringify(order) != '{}'">
		...
		<view class="order">
			...
			<view class="bottom">
				...
				<view v-if="order.status == 0" class="evaluate btn" @click.stop="payOrder(order.order_token)">去支付</view>
				<view v-if="order.status == 0" class="evaluate btn margin-left-20" @click.stop="cancelOrder(order.orderid)">
					取消订单</view>
				<view v-if="order.status == 4" class="evaluate btn">确认收货</view>

				<view v-if="order.status == 9" class="evaluate btn margin-left-10" @tap="refund(order.orderid)">退款</view>
	...
	methods: {
		...
		refund(orderid) {
			...
			uni.showModal({
				...
				success: function (res) {
					if (res.confirm) {
						_this.$store.dispatch('user/userRefund', { 'orderid': orderid }).then((res) => {
							if (res.code == 0) {
								_this.$refs.uToast.show({
									title: res.message
								})
