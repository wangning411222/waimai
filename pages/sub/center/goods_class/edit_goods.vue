		...
		<u-form :model="form" ref="uForm" :border-bottom="false">
			...
			<view class="card-list">
				...
				<u-form-item label="商品描述" label-width="170rpx" left-icon="edit-pen">
					<u-input v-model="form.goods_desc" type="text" placeholder="请输入描述" />
				</u-form-item>
			...
			<view class="card-list" style="padding: 35rpx 25rpx">
				<view class="d-flex" style="padding: 0 0 15rpx">
					...
					<view style="padding: 0 0 0 15rpx"><u-icon name="plus-circle" size="36" @click="skuAdd"></u-icon>
					</view>
				</view>
					...
					<view class="d-flex" style="padding: 0 0 15rpx;justify-content:space-between">
						...
						<view class="flex" style="display:flex;flex:1;justify-content:space-between;">
							<u-button type="error" size='mini' :plain="true"
								@click="DelSkuNoPrice(index)">移除类目</u-button>
							<u-button type="error" size='mini' :plain="true"
								@click="EditSkuNoPrice(index)">编辑类目</u-button>
						</view>
						<view style="padding: 0 0 0 15rpx;display:flex;flex:1;justify-content:flex-end;"><u-icon
								name="plus-circle" size="36" @click="skuNoPriceAdd(index, item)"></u-icon></view>
					</view>
					<view class="d-flex" v-for="(itemChildren, indexChildren) in item.skuNoPriceNum"
						:key="indexChildren" style="padding: 15rpx 0; border-bottom: 1rpx solid #efefef">
						<view class="flex">
							<u-input v-model="skuNopriceArr[index].skuArr[indexChildren].name" placeholder="规格名称" />
						</view>
						<view style="align-self: center; padding: 0 0 0 15rpx">
							<u-icon name="minus-circle" size="36"
								@click="skuNopriceDel(index, indexChildren, item)"></u-icon>
						</view>
					</view>
				</view>
			</view>
