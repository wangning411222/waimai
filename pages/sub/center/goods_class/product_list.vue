<template>
  <view>
    ...
    <view v-if="data.length > 0" style="padding-bottom: 106rpx">
      <view
        class="list_card u-padding-top-20 u-padding-bottom-20 u-padding-left-20 u-padding-right-20"
      ...
      >
          ...
          <view class="shop">
            <view>
              <view class="uni-title">
                ...
                <view class="is-recommend">
                  <text>是否热销:</text>
                  <u-switch
                    :value="item.is_recommend-0==0?false:true"
                    @change="isRecommend(item)"
                    active-color="#fa3534"
                    size="30"
                  ></u-switch>
                </view>
              ...
              <view>
                ...
                <text
                  v-if="item.is_own_business == 0"
                  class="shop-tag-shouquan"
                >
                  授权店
                </text>
            ...
            <view>
              <view style="display: flex; align-items: flex-end">
                ...
                <view
                  style="flex: 1; text-decoration: line-through; color: #bbbbbb"
                >
                  <text>¥</text>
              ...
              <view class="sales-box">
                ...
                <view v-if="item.stock == 0" class="sales-end">已售完</view>
                 <view 
              class="text-center rank-box"
              @click="setRank(item)"
              style="color: #aaa"
              >
              <text>排序</text><text>{{item.rank}}</text> </view
            >
              </view>
            </view>
          </view>
        ...
        <u-row class="u-padding-top-25" style="display: block">
          ...
          <u-col span="4">
            <view
              class="text-center"
              @click="goods_edit(item.goods_id)"
              style="color: #aaa"
              >编辑</view
            >
          </u-col>
          <u-col span="4">
            <view
              v-if="item.ischeck == 0"
              class="text-center"
              @click="goods_updown(item.goods_id, 1, index)"
              style="color: green"
              >上架</view
            >
            <view 
            span="4"
              v-if="item.ischeck == 1"
              class="text-center"
              @click="goods_updown(item.goods_id, 0, index)"
              style="color: red"
              >下架</view
            >
          </u-col>
          <u-col span="4">
            <view
              class="text-center"
              @click="goods_delete(item.goods_id, index)"
              style="color: #aaa"
              >删除</view
            >
        </u-row>
...
export default {
  ...
  Rankconfirm(){
    ...
    confirm() {
      let _this = this;
      _this.$store
        .dispatch("user/goodsDelete", { goods_id: this.active_goods_id })
        .then((res) => {
          if (res.code == 0) {
    ...
    goods_updown(goods_id, ischeck, index) {
      let _this = this;
      _this.$store
        .dispatch("user/goodsIscheck", { goods_id: goods_id, ischeck: ischeck })
        .then((res) => {
          if (res.code == 0) {
