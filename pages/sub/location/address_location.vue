<template>
	<view>
		...
		<view v-if="!search_show">
			...
			<view class="padding-20">
				...
				<view>
					...
					<view v-for="(item,index) in schoolList" :key="index" @tap="handlebuilding(index)" class="d-row margin-bottom-20 padding-top-20 padding-bottom-20">
		...
		<view v-if="search_show">
			<view v-for="(item,index) in searchList" :key="index" @tap="handlebuilding(index)" class="d-row padding-30">
	...
	export default {
		...
		methods: {
			...
			confirm(e) {
				...
				let floor = e[0]['label']
				let {schoolname,schoolid} = this.schoolList[this.activeSchoolIndex]
				let address = floor+' '+schoolname
				
				// 调用上一个页面的 uplist 方法 
				goBack('seTaddress',{'schoolname': schoolname,'floor': floor,'address': address,'schoolid':schoolid,'buid':e[0]['value']})
